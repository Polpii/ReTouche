...
export default function SecondScreen() {
  ...
  if(recCal){
    const W=640, k=W/recCal.baseWidth, H=recCal.baseHeight*k;
    absCrop={
  ...
  const saveRecPts = (pts:Points) => {
    ...
    if(currentLearnerName)
      savePositionConfig(currentLearnerName,soundId,{position:{x:0,y:0},perspectivePoints:pts}).catch(console.error);
    localStorage.setItem("recordedVideoPerspectivePoints",JSON.stringify(pts));
  };
  const saveNotePts = (pts:Points) => {
    setNotePtsAbs(pts);
    if(currentLearnerName)
      savePositionConfig(currentLearnerName,soundId+"-notes",{position:{x:0,y:0},perspectivePoints:pts}).catch(console.error);
