...
const stopPerformanceRecording = () => {
  ...
  const loopMidiEventsRef = useRef<any[]>([]);
...
// Fonction pour démarrer/arrêter l'enregistrement d'une loop
  ...
  if (!isLooping) {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        // Créer un nouveau layer avec vidéo et événements MIDI
        const newLayer = {
          videoUrl,
          midiEvents: [...loopMidiEventsRef.current],
          startTime: loopStartTimeRef.current
        };
        
        // Ajouter cette couche à nos layers existants
        const updatedLayers = [...loopLayers, newLayer];
        setLoopLayers(updatedLayers);
        
        // Sauvegarder dans le localStorage et IndexedDB
        try {
          // Stocker les métadonnées (sans les blobs vidéo) dans localStorage
          localStorage.setItem("mirrorFugueLoopLayers", JSON.stringify(updatedLayers.map(layer => ({
            ...layer,
            videoUrl: `loop_${updatedLayers.indexOf(layer)}` // Utiliser un identifiant simple
          }))));
          
          // Stocker le blob vidéo dans IndexedDB
          await saveLoopToIndexedDB(`loop_${updatedLayers.length - 1}`, blob);
        } catch (error) {
          console.error("Erreur lors de la sauvegarde des loops:", error);
        }
      ...
      // Réinitialiser les événements MIDI pour cette couche
      loopMidiEventsRef.current = [];
      loopStartTimeRef.current = performance.now();
      
      // Configurer la capture MIDI
      if (midiInputRef.current) {
        midiInputRef.current.onmidimessage = (event) => {
          // Vérifier que event.data n'est pas null
          if (event.data) {
            const timestamp = performance.now() - loopStartTimeRef.current;
            loopMidiEventsRef.current.push({
              data: Array.from(event.data),
              timestamp,
            });
            
            // Si on est en train d'enregistrer une loop par dessus une autre,
            // on veut aussi jouer les événements MIDI des couches précédentes
            if (loopLayers.length > 0 && midiOutputRef.current) {
              midiOutputRef.current.send(Array.from(event.data));
            }
          }
        };
      }
      ...
      console.error("Erreur lors du démarrage de l'enregistrement de la loop:", err);
    }
  } else {
...
// Fonction pour démarrer la lecture des loops
const startLoopPlayback = async (layers: any[]) => {
  ...
  try {
    ...
    if (videoBlob) {
      ...
      // Référence au temps de démarrage
      const loopStartTime = performance.now();
      
      // Lire les événements MIDI de toutes les couches
      const allMidiEvents = layers.flatMap(layer => layer.midiEvents);
      ...
      const maxDuration = Math.max(...allMidiEvents.map(e => e.timestamp), 1000) + 2000; // +2s pour s'assurer que tout est joué
      
      // Référence pour les fonctions de lecture programmées
      const scheduledFns: (() => void)[] = [];
      
      // Programmer la lecture des événements MIDI
      allMidiEvents.forEach(event => {
        ...
        setTimeout(fn, event.timestamp);
        scheduledFns.push(fn);
      });
      
      // Programmer la fin de la boucle et le redémarrage
      const loopTimeout = setTimeout(() => {
        // Redémarrer la vidéo
        if (loopVideoRef.current) {
          loopVideoRef.current.currentTime = 0;
          loopVideoRef.current.play().catch(err => console.error("Erreur lors de la lecture de la vidéo en boucle:", err));
        }
        
        // Rejouer tous les événements MIDI
        allMidiEvents.forEach(event => {
          setTimeout(() => {
            if (midiOutputRef.current && Array.isArray(event.data)) {
              midiOutputRef.current.send(event.data);
      ...
      loopIntervalRef.current = loopTimeout;
...
// Fonction pour arrêter la lecture des loops
const stopLoopPlayback = () => {
  ...
  // Tout arrêter (toutes les notes off)
  if (midiOutputRef.current) {
    midiOutputRef.current.send([0xB0, 123, 0]);
  }
              ...
              >
                ...
                {isLocalVideo ? (
                  <video
                    ...
                    autoPlay={false} // Changer à false pour contrôler manuellement le démarrage
                    playsInline
                    loop={isLoopPlaying} // Nous permettons à la vidéo de boucler 
                    style={{
                      position: "absolute",
                    ...
                    onEnded={() => {
                      // Si on est en mode looping, on redémarre la vidéo 
                      if (isLoopPlaying && loopVideoRef.current) {
                        loopVideoRef.current.currentTime = 0;
